...
client.on("messageCreate", async (message) => {
    ...
    if (command) {
        ...
        if (!botMember.permissions.has(["SEND_MESSAGES", "EMBED_LINKS"])) return errorEmbed(message, "Bot tidak memiliki akses ke `SEND MESSAGES` Dan `EMBED LINK`");
        ...
        if (!botMember.permissions.has(command.BotPerms || [])) return errorEmbed(message, `Bot tidak memiliki akses permissions \`${command.BotPerms || []}!\``);
    }
