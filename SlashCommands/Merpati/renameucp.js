...
module.exports = {
    ...
    run: async (client, interaction, args) => {
        ...
        MysqlMerpati.query(`SELECT * FROM ucp WHERE discordID = '${getAccount.id}'`, async (err, roww) => {
            if (roww[0]) {
                MysqlMerpati.query(`SELECT * FROM ucp WHERE username = '${getName}'`, async(err, row) => {
                    if(row[0]) {
                        ...
                        MysqlMerpati.query(`UPDATE ucp SET username = '${getName}' WHERE discordID = '${getAccount.id}'`);
                    }
                })
            } else {
